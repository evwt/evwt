  ...
  let menuVm = new Vue({
    ...
    methods: {
      ...
      handleBuild() {
        this.$on('build', async ({ id, menu }) => {
          ...
          this.$watch(() => this.menus[id], async (newMenu) => {
            ...
            loglevel.debug(`Change! sending evcontextmenu:set ${id}`);
      ...
      async syncMenu(id, newMenu) {
        ...
        loglevel.trace(serializedNewMenu);
        ...
        // menu items.
        let changes = deepDiff(this.menus[id], setMenu) || [];
        for (const { kind, path, rhs } of changes) {
          if (kind === 'N') {
            loglevel.debug('Setting new property from background onto menu item');
      ...
      handleInput() {
        ipcRenderer.on('evcontextmenu:ipc:input', (e, { id, item }) => {
          ...
          if (!menu) {
            loglevel.warn(`[EvContextMenu] Could not find menu from id ${id}`);
            return;
          }

          loglevel.debug('Got menu', menu);

          // Get the menu item in our collection
          let menuItem = this.getItem(id, item.id);

          loglevel.debug('Got menu item', menuItem);

          // set the item's properties from the payload
          for (let key of Object.keys(item)) {
            loglevel.debug(`setting ${key} to ${item[key]}`);
...
export { EvContextMenu, __vue_component__ as EvDropZone, __vue_component__$1 as EvIcon, __vue_component__$3 as EvLayout, EvMenu, EvStore, __vue_component__$4 as EvToolbar, __vue_component__$5 as EvToolbarItem, __vue_component__$6 as EvVirtualScroll };
