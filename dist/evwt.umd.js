	    ...
	    methods: {
	      ...
	      handleBuild() {
	        this.$on('build', async ({ id, menu }) => {
	          ...
	          let builtMenu = await electron.ipcRenderer.invoke('evcontextmenu:build', { id, menu });
	      ...
	      async syncMenu(id, newMenu) {
	        ...
	        let setMenu = await electron.ipcRenderer.invoke('evcontextmenu:set', { id, menu: serializedNewMenu });
	      ...
	      handleShow() {
	        this.$on('show', async id => {
	          await electron.ipcRenderer.invoke('evcontextmenu:show', id);
	        });
	      },
